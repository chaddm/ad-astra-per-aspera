...
When piping `things inbox --json` through `toon`, the command returns data in **TOON (Token-Oriented Object Notation)** format - a compact, human-readable, schema-aware format optimized for LLM token efficiency.
...
**Note:** The exact fields returned depend on the data and options used. TOON format adapts to include only the fields that have data.
...
1. **Always pipe through toon:** Use `things inbox --json | toon` for token-efficient output
...
For Things3 inbox data, which typically consists of uniform todo objects, TOON format provides significant token savings while maintaining readability and structure.