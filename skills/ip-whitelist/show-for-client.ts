#!/usr/bin/env bun
/**
 * Show whitelisted IPs for a specific client (bun version)
 * Usage: show-for-client --env <env> --client-id <client_id>
 */
...
// Parse --env and --client-id arguments (allow --key=val or --key val)
let env, clientId;
for (let i = 0; i < argv.length; i++) {
  ...
  if (arg === '--client-id') {
    if (argv[i + 1]) {
      ...
      printErr('Missing value after --client-id', 2);
    }
  }
}
...
try {
  ...
  // Compose URL
  const url = baseUrl.replace(/\/$/, '') + `/api/admin/clientIps/` + encodeURIComponent(clientId);
