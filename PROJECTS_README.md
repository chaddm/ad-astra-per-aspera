...
and tools, and ensuring the integrity of the `opencode` setup. Do not place
project-specific configuration files here; those belong in the `.opencode/` directory
within each project.
...
1. **Configuration Management**:
   ...
   - Ensure all changes are backward-compatible and do not disrupt existing
     functionality.
...
2. **Agent and Tool Management**:
   ...
   - Manage custom commands in the `command/` directory for extending OpenCode
     functionality.
   - Ensure all agents and tools are properly documented and adhere to the `opencode`
     standards.

3. **Integrity and Validation**:
   - Validate configuration changes to ensure they meet the application's
     requirements.
   - Test changes thoroughly to prevent regressions.
   - Ensure that all changes are compatible with the automatic build and test systems
     of the `opencode` application.
...
2. **Formatting**:
   ...
   - Use JSON or YAML for configuration files, depending on the application's
     requirements.
...
5. **Documentation**:
   ...
   - Include comments within configuration files where necessary to explain complex
     settings.
...
1. **`agents.md`**:
   ...
   - **Content**: Overview of agents, their types (primary and subagents), and usage
     instructions.
