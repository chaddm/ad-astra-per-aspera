...
import type { TextReadParams, TextPatchParams, TextReadFrontmatter, PatchOperation } from "../tool/text-patcher.types"
...
describe("text-patcher tool", () => {
  ...
  describe("default tool", () => {
    ...
    it("has correct description", () => {
      expect(textPatcher.description).toBe("Handle file reading and writing through operational transforms")
    })
  })
  ...
  describe("text_read sub-tool", () => {
    describe("positive scenarios", () => {
      ...
      it("returns token as null for non-existent file", async () => {
        const result = await text_read.execute({ filename: NONEXISTENT_FILE }, {} as any)
  ...
  describe("text_patch sub-tool", () => {
    describe("positive scenarios", () => {
      ...
      it("creates new file with token: null", async () => {
        const readResult = await text_read.execute({ filename: NONEXISTENT_FILE }, {} as any)
        const token = parseFrontmatter(readResult)?.token
      ...
      it("handles complex patch sequence with original row referencing", async () => {
        ...
        const verifyResult = await text_read.execute({ filename: TEST_FILE }, {} as any)
        const lines = extractContent(verifyResult).map(line => line.replace(/^\d{5}\|/, ""))
    ...
    describe("atomicity", () => {
      it("applies all patches or none (all-or-nothing)", async () => {
        ...
        const verifyResult = await text_read.execute({ filename: TEST_FILE }, {} as any)
        const verifyContent = extractContent(verifyResult).map(line => line.replace(/^\d{5}\|/, "")).join("\n")
